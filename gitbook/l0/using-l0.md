...
| L1 overlay TCP byte-stream (`conet-l0d`) | Catch overlay `100.64.0.0/10` and carry geth / beacon TCP on existing `/post` | Crate MVP accepted; P1 outbound + inbound decrypt/TUN write-back in-crate, default off; lab binary may run with `[l0]` off; live mailbox SSE not opened. No live SI `p2p_stream_*` / `listenKind: "l1p2p"`. Keep public P2P for the 6s slot |
