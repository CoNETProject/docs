...
| Duplex overlay | Offer on long-lived Chat SSE; accept / reject / AES `duplex_frame` on session listen SSEs | `duplex_reject` or missing accept keeps P1 gossip. SI releases occupied pipes with `l0_pipe_end` / `l0_listen_released` on teardown. SI has **no** duplex pool. Spec: [Duplex overlay](../l0/duplex-forward.md) |
