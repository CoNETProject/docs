...
| L1 overlay (`conet-l0d`) | Existing `{ data }` `/post`. Crate MVP accepted; P1 outbound + inbound decrypt/TUN write-back in-crate, default off; lab binary may run with `[l0]` off; live mailbox SSE not opened. Catch overlay `100.64.0.0/10` only | [Developers — conet-l0d](conet-l0d.md). No live SI `p2p_stream_*` / `listenKind: "l1p2p"` |
