...
| Duplex overlay | Offer on long-lived Chat SSE; accept / reject / AES `duplex_frame` on session listen SSEs | `duplex_reject` or missing accept keeps P1 gossip. SI has **no** duplex pool. Spec: [Duplex overlay](../l0/duplex-forward.md) |
...
| L1 overlay (`conet-l0d`) | Application duplex on Chat gossip when the peer app sends `duplex_accept`; P1 gossip (user-PGP envelope + mailbox wrap) otherwise. Crate MVP accepted; `[l0]` default off; authorized lab may enable it. 2026-08-18: overlay geth + beacon TCP; follow-the-chain Prysm-bound; lab discv5 via L0 accepted (not a production product). HTTP 200 ≠ delivery. Catch overlay `100.64.0.0/10` only | [Developers — conet-l0d](conet-l0d.md). Do **not** treat SI `duplex_*` / `p2p_stream_*` / `listenKind: "l1p2p"` as current SI |
