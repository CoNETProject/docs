...
| Laboratory honesty track P12–P25 | Engine + `154/154` tests. P23 keep-deploy **6/7 `LIVE_OK`** + fd-01 409→accept; fd-06 HTTP unstable. P24 isolated `node.ts` standby gate. **P25 landed:** Certificates + Home **non-green** overlays; public SPA `index-U1o9ul_I.js` on [dle.conet.network](https://dle.conet.network/) (2026-08-18T00:18:49Z); live `/health` at publish still `officialStandbysReady=false` (`count=1`); seating copy is **lab EIP-712**, never HMAC; green pills stay `seatingQualified === true` only (`explorer:test` 8/8) — [Lab honesty track](lab-honesty-track.md) |
