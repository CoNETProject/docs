...
- `conet-l0d` crate MVP is accepted. P1 encrypt + mailbox wrap + POST, inbound decrypt + TUN write-back, and a listen HTTP+SSE worker exist in-crate and stay off. The listen command is unsigned; production SI `checkSign` will reject. A lab host may run that binary with `[l0]` still off. Production SI listen, live overlay TCP on production SI, and measured slot RTT are not available. Keep public P2P for attestations. Do not advertise overlay vIPs until a bidirectional frame is proven. Do not use SilentPass as L1 peering.
