...
| **L1 overlay daemon (conet-l0d)** | Linux operator command for geth / beacon overlay P2P | TUN + owned iptables catch overlay `100.64.0.0/10`; `web3://` locators; crate MVP accepted; P1 outbound + inbound decrypt/TUN write-back + EIP-191 listen worker + SI gossip JSON ingest in-crate; `[l0]` default off; authorized lab may enable `[l0]`; 2026-08-18: `.45` advertises overlay vIP; overlay geth + beacon TCP; CL initial-sync in progress. Not SilentPass egress | **Under development** | [L1 overlay daemon](conet-l0d.md) | [CoNET-project/CoNET-L0D](https://github.com/CoNET-project/CoNET-L0D); no live SI `p2p_stream_*`; HTTP 200 ≠ delivery; authorized L0_ONLY `.45` may advertise overlay vIP; production proposers keep public IP |
