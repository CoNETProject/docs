...
| **Membership** | Selector shows **base membership (index 0)** plus each higher paid tier. A new BeaconProxy card creates its complete ordered tier, fee, and duration schedule in the same create receipt; metadata supplies presentation only. The terminal uses the on-chain fee schedule and rejects mismatched amounts. A direct membership purchase charges **fee only** (two-decimal display), mints a membership NFT with `tokenId ∈ [100, 1e11)`, and grants **no** `#0` program credit. Cashiers must make a separate Top-up after membership is issued. |
